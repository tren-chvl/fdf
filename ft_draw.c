...
void draw_horizontal(t_data *d, int i, int j)
{
    ...
    if (j + 1 >= d->map->col)
        return ;
    p1 = project_iso(d->map->points[i][j], d, d->scale, d->offset_x, d->offset_y);
    p2 = project_iso(d->map->points[i][j + 1], d, d->scale, d->offset_x, d->offset_y);
...
void draw_vertical(t_data *d, int i, int j)
{
    ...
    if (i + 1 >= d->map->line)
        return ;
    p1 = project_iso(d->map->points[i][j], d, d->scale, d->offset_x, d->offset_y);
    p2 = project_iso(d->map->points[i + 1][j], d, d->scale, d->offset_x, d->offset_y);
    color = get_color(d->map->points[i][j]);
    draw_line(d, p1, p2, color);
}
